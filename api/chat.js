export default async function handler(req, res) {
  ...
  // Gemini يفهم الطلب ويقرر هيسرش ولا هيرد بس
    ...
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${process.env.GEMINI_KEY}`,
    {
      ...
      body: JSON.stringify({
        contents: [
          {
            role: 'user',
            parts: [{ text: `You are a smart shopping assistant. You help users find and compare products.
...
IMPORTANT: Always respond in the SAME language the user used. If they write in Arabic, reply in Arabic. If English, reply in English.
...
User message: "${message}"` }]
  ...
  if (parsed.action === 'search') {
    // SerpAPI تجيب المنتجات
    let serpUrl = `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(parsed.query)}&api_key=${process.env.SERPAPI_KEY}`;
